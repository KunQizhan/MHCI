...
15. Bugfix: Firebase upload failing with "value argument contains undefined in property targetID". Root cause: feedback overlay had pointer-events:none, so taps could register during the 600ms feedback, pushing trialIdx out of bounds. Fixed with three layers: (a) feedbackActive guard blocks all input during feedback, (b) recordTrial skips if targetIdx is undefined, (c) JSON.parse(JSON.stringify()) strips any remaining undefined before Firebase set().
16. Final state: prototype ready for deployment. Trial flow: 4 practice + 16 formal per condition × 3 conditions = 60 task trials + 39 questionnaire items. ~10–15 min per participant.